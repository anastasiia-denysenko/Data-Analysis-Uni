...
Після запуску програми ви побачите поле з троьма графіками.
Червоний графік - графік сигналу з гаусовським шумом,
чорний - графік сигналу без шуму і синій- графік відфільтрованого сигналу.
Змінивши значення на синіх слайдерах, Ви можете побачити
як змінюється графік сигналу з шумом та відфільтрований графік,
...
def update(val):
    ...
    l1.set_ydata(noise*np.sin(freq*t1+noise)+gaussian_noise*varience)
    l2.set_ydata(filtfilt(x, y, (noise*np.sin(freq*t1+noise)+gaussian_noise*varience)))
